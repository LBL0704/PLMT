...
def get_r_adv_t(x, decoder, it=1, xi=1e-1, eps=10.0):
    # stop bn
    decoder.eval()

    x_detached = []
    for feature in x:
        x_detached.append(feature.detach())
    with torch.no_grad():
        # get the ensemble results from teacher
        pred = F.softmax(decoder(x_detached), dim=1)

    d = torch.rand(x[-1].shape).sub(0.5).to(x[-1].device)
    d = _l2_normalize(d)

    # assist students to find the effective va-noise
    for _ in range(it):
        d.requires_grad_()
        x_detached[-1] = x_detached[-1] + xi * d
        pred_hat = F.softmax(decoder(x_detached), dim=1)
...
class Decoder_DTC(nn.Module):
    ...
    def forward(self, feature):
        ...
        output = self.out_conv2(x)

        return output, out_tanh
...
class UNet_VAT(nn.Module):
    def __init__(self, in_chns, class_num):
        super(UNet_VAT, self).__init__()

        self.iter_threshold = False
        params = {'in_chns': in_chns,
                  'feature_chns': [16, 32, 64, 128, 256],
                  'dropout': [0.05, 0.1, 0.2, 0.3, 0.5],
                  'class_num': class_num,
    ...
    def forward(self, x):
        feature = self.encoder(x)
        if self.iter_threshold is True:
            if self.training is True:
                r_adv = get_r_adv_t(feature, self.decoder, it=1, xi=1e-6, eps=2.0)
                feature[-1] = feature[-1] + r_adv
        output = self.decoder(feature)
        return output
...
class UNet_DTC(nn.Module):
    ...
    def forward(self, x):
        feature = self.encoder(x)
        output, out_tanh = self.decoder(feature)
        return output, out_tanh
